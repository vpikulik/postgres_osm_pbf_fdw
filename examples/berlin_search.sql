...
CREATE OR REPLACE FUNCTION text_index_from_json(data json) RETURNS text AS $$
    SELECT string_agg(data->>key, ' ')
        FROM (SELECT json_object_keys(data) as key, data) as data_keys;
